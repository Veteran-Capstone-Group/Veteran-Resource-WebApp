...
export const Header= () => {
	...
	if(categoryOne) {
		...
		selectedCategory = window.location.pathname === "/" ? "Home" : window.location.pathname === "/data-resources" ? "Data Resources" : categories.find(category => category["categoryId"] === window.location.pathname.split("/Category/")[1])["categoryType"];
	}
